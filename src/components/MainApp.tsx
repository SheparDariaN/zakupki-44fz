        ...
        <aside className="col-span-1 xl:col-span-4 flex flex-col gap-6 overflow-hidden scroll-area pr-2">
          ...
          <section className="bg-surface/50 p-5 border border-line shrink-0 shadow-sm transition-all hover:bg-surface/80">
            <h2 className="text-[11px] uppercase font-bold mb-4 flex items-center gap-2">
              <span className="w-2 h-2 bg-ink rounded-full animate-pulse"></span> Реквизиты Закупки
            </h2>
              ...
              <table className="w-full data-grid border-none">
                ...
                <tbody>
                  {state.positions.map((position, positionIndex) => (
                    <tr key={position.id} className="group hover:bg-ink/5 transition-colors">
                      ...
                      <td className="text-center p-0 align-middle">
                        <button onClick={() => removePosition(position.id)} className="text-ink/30 hover:text-red-600 hover:bg-red-50 p-1.5 rounded transition-all opacity-0 group-hover:opacity-100 mx-auto" title="Удалить">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </td>
                    </tr>
          ...
          <section className="border border-line bg-surface/50 flex flex-col shrink-0 shadow-sm">
            <div className="p-3 border-b border-line shrink-0 bg-ink text-page">
              ...
              <table className="w-full data-grid border-none whitespace-nowrap">
                ...
                <tbody>
                  {state.positions.map((pos, positionIndex) => (
                    <tr key={pos.id} className="hover:bg-ink/5 transition-colors">
                        ...
                        return (
                          <td key={sup.id} className="mono p-0 text-right">
                            <input 
                              ...
                              inputMode="decimal"
                              placeholder="0.00"
                              className="w-full h-full bg-transparent border border-transparent hover:bg-blue-50 focus:bg-surface focus:border-blue-500 outline-none px-2 py-2 text-right text-[11px] transition-all cursor-text"
                              value={val}
...
function PreviewContent({ state }: { state: AppState }) {
  ...
  return (
    <div className="paper-sheet animate-in fade-in duration-300 box-border w-[297mm] min-w-[297mm] min-h-[210mm] mx-auto bg-white shadow-[0_10px_30px_rgba(0,0,0,0.1)] p-[10mm] text-[10pt] font-serif leading-tight">
